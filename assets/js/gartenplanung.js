...
class Component extends DCLogic {
  ...
  componentDidMount() {
    ...
    if (form) form.addEventListener('submit', e => {
      ...
      const mailto = () => {
        window.location.href = 'mailto:' + ((window.dvFormular && window.dvFormular.empfaenger) || 'info@devries-galabau.de') + '?subject=' + encodeURIComponent('Anfrage Gartenplanung') + '&body=' + encodeURIComponent(body);
      };
      /* Zentrale Stelle: speichert, falls eingerichtet — sonst mailto. */
      if (window.dvFormular) window.dvFormular.senden({
        form: form, quelle: 'gartenplanung', betreff: 'Anfrage Gartenplanung',
        felder: {
